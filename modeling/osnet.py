...
class OSNet(nn.Module):
    ...
    def __init__(self, num_classes, blocks, layers, channels, feature_dim=512, loss='softmax', IN=False, **kwargs):
        super(OSNet, self).__init__() # model_path, pretrain_choice
        ...
        self.classifier_person_reid = nn.Linear(self.feature_dim, num_classes)

        # weakly branch：
        self.cls = self.branch_weakly(feature_dim, num_classes)
        self.threshold = 0.6
    ...
    def branch_weakly(self, in_planes, out_planes):
    ...
    def forward(self, x,label, return_featuremaps=False):
        ...
        if not self.training:
            return v
        y = self.classifier_person_reid(v)  # global

        if self.training:
            # Branch weakly
            weakly_feat = self.cls(x)
            logits_1 = torch.mean(torch.mean(weakly_feat, dim=2), dim=2)
