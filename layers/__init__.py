...
from .triplet_loss import CrossEntropyLabelSmooth,ImprovedTripletLoss
...
def make_loss(cfg, num_classes):    # modified by gu
    ...
    if cfg.MODEL.METRIC_LOSS_TYPE == 'triplet':
        ...
        improved_tripletloss = ImprovedTripletLoss(cfg.SOLVER.MARGIN)
    ...
    if sampler == 'softmax':
        ...
        def loss_func(score, feat, target, logits_1):
            if cfg.MODEL.METRIC_LOSS_TYPE == 'triplet':
                if cfg.MODEL.IF_LABELSMOOTH == 'on':
                    return xent(score, target), improved_tripletloss(feat, target)[0], F.cross_entropy(logits_1, target)
                else:
